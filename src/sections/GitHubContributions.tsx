...
export default function GitHubContributions() {
  ...
  return (
    <section ref={sectionRef} className="py-8 md:py-12 px-4 md:px-8 bg-paper">
      <div className="max-w-7xl mx-auto">
        <h2
          ref={titleRef}
          className="text-3xl md:text-4xl font-mono font-bold mb-6 text-center"
        >
          {t('GitHub Activity', 'Aktivitas GitHub')}
        </h2>

        <div ref={graphRef} className="card-brutal p-4 md:p-5">
          {/* Header */}
          <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 border-2 border-[var(--ink)] flex items-center justify-center bg-[var(--ink)] text-[var(--paper)]">
              ...
              <a
                ...
                className="font-mono text-sm uppercase tracking-wider hover:bg-[var(--ink)] hover:text-[var(--paper)] transition-colors px-1"
              >
                @{GITHUB_USERNAME}
              </a>
            </div>
            {!loading && (
              <span className="tag-brutal bg-[var(--accent-green)] text-white">
          ...
          {/* Contribution Graph */}
          {loading ? (
            <div className="h-32 flex items-center justify-center font-mono text-sm text-[var(--ink-light)]">
            ...
            <div className="overflow-x-auto no-scrollbar">
              ...
              <div className="flex mb-1 ml-8" style={{ gap: '0px' }}>
                {monthLabels.map((label, i) => (
                  <span
                    key={i}
                    className="font-mono text-[10px] text-[var(--ink-light)] uppercase"
              ...
              <div className="flex gap-0">
                ...
                <div className="flex flex-col justify-around pr-2" style={{ height: `${7 * 16}px` }}>
                  {['Mon', 'Wed', 'Fri'].map((day) => (
                    <span key={day} className="font-mono text-[10px] text-[var(--ink-light)] leading-[13px]">
